...
⛔ **Alice is PRIMARY; `graphify-out/graph.json` is a CACHE.** Before querying the cache,
measure whether the substrate can answer (`graph_execute` `metrics` on the corpus
workspace, ~9ms). Non-zero words/joins ⟹ query Alice and let Alice win on conflict. Zero ⟹
the fold never ran; answer from the cache, SAY SO, and run Step 5.5.
