...
⛔ **EVERY EXPORT ON THIS PAGE IS AN EPHEMERAL, NON-AUTHORITATIVE PROJECTION.** The
authoritative store is Alice's JoinGraph, written by Step 5.5's fold. Neo4j, FalkorDB,
GraphML, the wiki and the MCP server are all *views* — none of them may be treated as the
source of truth, and none of them is a place to look something up that the substrate could
answer. If an export and Alice disagree, **Alice wins**; re-derive the export.
