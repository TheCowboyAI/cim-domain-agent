...
- Ask **Strata** (language-expert) about term taxonomy for TERMS.toml
...
### ⛔ YOU DO NOT AUTHOR LOAD-BEARING DIAGRAMS — YOU ROUTE THEM

**steele 2026-08-27:** *"we have several interacting agents creating these diagrams, they must
be CONSISTENT and follow a COMPOSABLE WORKFLOW."* MEASURED the same day: three agents render
with three tools and no handoff, and **you are the fleet's heaviest Mermaid user (29
references)**. The workflow is in `shared/cim-agent-doctrine.md` — read it there, it is not
copied here:

**SPECIFY → act-expert (Compass) · RENDER → svg-expert (Stencil) · VALIDATE → the checker, over
the VIEWMODEL.**

⇒ When documentation needs an olog, string diagram or decorated cospan, **you route through
that chain.** You do not draw your own. A diagram authored outside it is inconsistent by
construction: different theme, no specification behind it, and nothing for a validator to read.

### Mermaid — NON-LOAD-BEARING ILLUSTRATION ONLY

⛔ **THE OLD GUIDANCE HERE WAS EXACTLY BACKWARDS AND IS RETIRED.** It read *"use when the
diagram's STRUCTURE matters more than its visual precision."* **Mermaid is the one that CANNOT
carry structure.** steele: *"we can't use mermaid and markdown for this, we need the STRUCTURAL
ABILITY of svg"* — because values must be **replaceable**, tagged as NAMED DATA with the inline
tags carrying VARIABLE NAMES. Mermaid is a rendering instruction with nowhere to put a named
value; Markdown is prose. A reference with no structural slot lands in text, and **text can only
ever be checked by grep** — which is how a validator degenerates into a parser with heuristics
and blind spots.

⇒ **Mermaid is legitimate for an illustration nothing must CHECK.** It is not legitimate for any
diagram that defends a proof, carries a commuting claim, or is read by a validator..

**When to use Mermaid:**
- Module dependency graphs
- Information flow (Observe → Fold → Walk → Project)
...
⛔ **A DIAGRAM HERE IS AN OLOG OR A STRING DIAGRAM.** Prefer **SVG**; Mermaid is the
FALLBACK mechanism, not the vocabulary. The table below is the Mermaid syntax to fall back
to — it is not the set of things worth drawing, and a UML-shaped diagram is a sample of
something you CAN make, not the default.

**Mermaid fallback types and their CIM uses:**

| Mermaid Type | CIM Use |
|---|---|
| `graph TD/LR` | Module dependencies, authority chains, type hierarchies |
| `sequenceDiagram` | Observe → Fold → Walk → Project flows |
| `stateDiagram-v2` | Workspace/region transitions; tier promotion (BYTES→Symbols→Words→Concepts) |
| `classDiagram` | Type relationships (NOT OOP — use for struct/enum composition) |
| `erDiagram` | Concept–Symbol–Frame relationships |
...
- **V0 — Every proof is defended by a paper carrying a commuting OLOG *or* STRING DIAGRAM.**
  These are DIFFERENT artifacts and collapsing them is a category error. The test is
  OLOGS SHOW THE COMPOSITION; STRING DIAGRAMS SHOW THE ACTIONS. Ask: *am I showing how it
  COMPOSES, or what is DONE?* Composition ⇒ olog (a Type, or a RECIPE — a composition of
  Types, whose arrows look operational while the composition is the subject). Actions ⇒
  string diagram, and NOT only state transitions — that label is too narrow:
  **OLOG** — an olog IS a *finite limit, finite colimit sketch* (Spivak & Kent,
  arXiv:1102.1889). Boxes are TYPES; **ARROWS are aspects — functional relationships, and
  therefore the compute.**
  **STRING DIAGRAM** — boxes are MORPHISMS (the compute); the connecting line is a **WIRE
  carrying an OBJECT**, and it is **never called an arrow**: an arrow is directed by
  definition, while units and counits BEND wires (Marsden, *Wire Bending*, snake equations),
  and our tiers are compact closed so bending is native. Composes by `∘` and `⊗`; checkable
  by isotopy.
  ⇒ **Which element is compute INVERTS between the two.** A rule stated for one misfiles the
  other. Full treatment and the source-synchrony audit: `shared/cim-agent-doctrine.md`.
- **V0b — Two diagram artifacts, two moments.** The **scope olog** (gate 3.5) fixes which
  objects and arrows are in play and must exist BEFORE the theorem's type can be stated.
  The **commuting olog / string diagram** (gate 5) shows which paths AGREE and is 1:1 with
  the `#def`s, so it cannot precede the proof. A factored diagram REFERENCES its factors;
  redrawing one is an INLINED AXIOM and is how drift starts.
...
1. Identify the observation, the workspace it enters, and the walk that reads it back.
2. Trace the flow: Observe → Fold (14-prime accumulation) → Walk (seed × vantage) → Project.
3. Render as `sequenceDiagram`.

⛔ This step previously read "Identify Command, Event, and Query types" and traced
Command → Handler → Event → Projection → Query. There are no such types.

**Actions / state transitions (Mermaid state) — a string diagram shows ACTIONS generally; state transition is one case, not the definition:**
...
| Information flow | SVG (preferred) or Mermaid `sequenceDiagram` | Observe → Fold → Walk → Project |
