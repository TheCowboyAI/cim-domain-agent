...
use bevy::prelude::{App, MinimalPlugins, Time, Update, Startup};
use cim_domain_agent::{
    AgentEntity, AgentId, AgentCapabilities, AgentStatus, 
    AgentType, AgentTypeComponent,
    components::status::AgentState,
    systems::{
        ConceptualReasoningPlugin, ConceptualAnalysisRequest, ConceptualAnalysisResult,
        SimilaritySearchRequest, SimilaritySearchResult, ConceptualReasoningAgent,
        initialize_conceptual_reasoning_system, process_conceptual_analysis_system,
    },
    integration::{GraphConceptualMapper, GraphMetrics, GraphContentSummary, NodeMetrics},
    value_objects::AnalysisCapability,
};
use cim_domain_graph::{GraphId, components::NodeContent};
...
fn setup_demo_agent(mut commands: Commands) {
    ...
    info!("Created agent {:?} with conceptual reasoning capabilities", agent_id);
}
...
fn handle_analysis_results(
    mut analysis_results: EventReader<ConceptualAnalysisResult>,
) {
    for result in analysis_results.read() {
        ...
        info!("Conceptual position: {:?}", result.analysis.conceptual_position);
        ...
        for recommendation in &result.analysis.recommendations {
            info!("Recommendation: {} (confidence: {:.2})", 
...
fn demonstrate_graph_mapping(
    mut demonstrated: Local<bool>,
) {
    if *demonstrated {
        return;
    }
    ...
    let node_content = NodeContent {
        ...
        description: "Connect to external payment service API for transaction processing".to_string(),
