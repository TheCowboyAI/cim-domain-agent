...
fn handle_keyboard_input(
    ...
    if keyboard.just_pressed(KeyCode::Space) || keyboard.just_pressed(KeyCode::Digit1) {
        ...
        run_analysis = Some(AnalysisCapability::Custom(
    ...
    if let Some(capability) = run_analysis {
        ...
        if let Ok(result) = pollster::block_on(future) {
            ...
            if let Ok(mut text) = text_query.get_single_mut() {
                **text = format!("CIM AI Visual Demo - Provider: {ai_provider.provider_name}\n\nAnalysis: {:?}\n{capability}\n\nInsights:\n{result.summary}\n\nPress SPACE to analyze\nPress 1-4 for different analyses", result.insights.iter()
...
fn update_highlights(
    ...
    for (material_handle, highlighted) in query.iter_mut() {
        if let Some(material) = materials.get_mut(&material_handle.0) {
            ...
            material.emissive =
                LinearRgba::from(highlighted.color) * highlighted.intensity * pulse * 2.0;
        }
    }
}
