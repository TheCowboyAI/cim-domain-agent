...
use bevy::ui::{BackgroundColor, PositionType, UiRect, Val};
...
fn setup(
    ...
    let provider_type = std::env::var("DEFAULT_AI_PROVIDER").unwrap_or_else(|_| "mock".to_string());
    ...
    let provider =
        AIProviderFactory::create_provider(&config).expect("Failed to create AI provider");
    ...
    // UI setup
    let ui_text = format!("CIM AI Visual Demo - Provider: {provider_name}\n\nPress SPACE to analyze\nPress 1-4 for different analyses\nMouse to rotate camera\nScroll to zoom");
...
fn camera_controller(
    ...
    if mouse_button.pressed(MouseButton::Left) {
        for event in mouse_motion.read() {
            ...
            controller.phi =
                (controller.phi - event.delta.y * 0.01).clamp(0.1, std::f32::consts::PI - 0.1);
        }
    }
...
fn handle_keyboard_input(
    ...
    let capability =
        if keyboard.just_pressed(KeyCode::Space) || keyboard.just_pressed(KeyCode::Digit1) {
            ...
            Some(AnalysisCapability::Custom(
                "Find bottlenecks and suggest improvements".to_string(),
            ))
        } else {
            None
        };
...
fn rotate_nodes(mut query: Query<&mut Transform, With<GraphNode>>, time: Res<Time>) {
...
fn update_ui_text(
    ...
    for mut text in text_query.iter_mut() {
        let mut content = format!("CIM AI Visual Demo - Provider: {ai_provider.provider_name}\n\nPress SPACE to analyze\nPress 1-4 for different analyses\nMouse to rotate camera\nScroll to zoom\n\n");
        ...
        if let Some(result) = &analysis_results.current {
            ...
            if !result.insights.is_empty() {
                ...
                for insight in &result.insights {
                    content.push_str(&format!(
                        "• {insight.description} (confidence: {:.2})\n",
                        insight.confidence
                    ));
                }
                content.push('\n');
            ...
            if !result.recommendations.is_empty() {
                ...
                for rec in &result.recommendations {
                    content.push_str(&format!("• {rec.title}: {rec.description}\n"));
                }
            }
        }
...
fn create_sample_workflow() -> GraphData {
    let nodes = vec![
        ...
        NodeData {
            ...
            properties: HashMap::from([("duration_ms".to_string(), json!(500))]),
        ...
        NodeData {
            ...
            properties: HashMap::from([("duration_ms".to_string(), json!(2000))]),
            position: Some((0.0, 0.0, 3.0)),
        },
        NodeData {
            id: "ship".to_string(),
