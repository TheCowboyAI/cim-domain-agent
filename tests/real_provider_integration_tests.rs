    ...
    GraphData {
        ...
        nodes: vec![
            ...
            NodeData {
                ...
                properties: HashMap::from([("duration".to_string(), json!("10m"))]),
        ...
        metadata: HashMap::from([("test".to_string(), json!(true))]),
    }
}
...
mod openai_tests {
    ...
    async fn test_openai_connection() {
        ...
        let provider =
            AIProviderFactory::create_provider(&config).expect("Failed to create OpenAI provider");
    ...
    async fn test_openai_workflow_optimization() {
        ...
        let result = provider
            .analyze_graph(
                ...
                HashMap::from([("focus".to_string(), json!("performance"))]),
            )
            .await;
...
mod anthropic_tests {
    ...
    async fn test_anthropic_connection() {
        ...
        let graph = create_test_graph();
        let result = provider
            .analyze_graph(graph, AnalysisCapability::PatternDetection, HashMap::new())
            .await;
...
mod ollama_tests {
    ...
    async fn test_ollama_connection() {
        ...
        let config =
            create_provider_config("ollama", None, Some("llama3.2".to_string()), Some(base_url))
                .expect("Failed to create config");

        let provider =
            AIProviderFactory::create_provider(&config).expect("Failed to create Ollama provider");

        let graph = create_test_graph();
        let result = provider
            .analyze_graph(graph, AnalysisCapability::SemanticAnalysis, HashMap::new())
            .await;
